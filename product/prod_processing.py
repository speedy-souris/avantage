"""this module manages all the methods for the product selection menus
by displaying the product characteristics recorded in the database"""
...
from prod_methode import recall_choice as r_choice
from prod_methode import recall_chosen as r_bad
from prod_methode import recall_nb as r_nb
from prod_methode import recall_val as r_val
...
def choice(d_json, cat_name, cat_id, db_con, nb_product, product_list):
    ...
    Funcs:
        substitutes_display() alias sub_display ==> display module and choice
        ...
        continuity() alias cont ==> module of choice
...
# +----------------+
def choice_2(
        d_json, product, cat_id, db_con,
        product_display, cursor, selection
):
    ...
    Args:
        ...
        cursor         ==> content of the selection
                            of substituted products in the database

        selection      ==> memorization of the product choice
    ...
    Queries:
        sql_selected   ==> SQL query that selects
                            the substituted product id chosen"""

    while True:
        while True:
            print()
            sub_select = input(
                "choississez un produit par son numéro : "
            )
            ...
            try:
                ...
                substitutes_display(d_json, product, cat_id, db_con, "o")
    ...
    if selection.lower() == "n":
        cursor.close()
        print()
        print("Vous n'avez choisi aucun produit a sauvegarder !")
...
# |  SUBSTITUTE  |
# +--------------+
def substitutes_display(d_json, product, cat_id, db_con, select=""):
    ...
    Args:
        ...
        select         ==> memorization of the acceptance
                            of the product choice after a wrong choice
                            in the product list
    ...
    for elt_sub3 in substitute_list[:3]:
        ...
        while nb_val < 6:
            for sub3_val, sub3_data in elt_sub3[nb_val].items():
                print(f"{sub3_val.upper()} : {sub3_data}")
                if nb_val == 1:
    ...
    if select == "":
        while True:
            ...
            while True:
                ...
                if selection not in "on":
                    r_bad()
                    substitutes_display(d_json, product, cat_id, db_con)

                # acceptance for product choice
                elif selection.lower() == "o":
                    id_sub = choice_2(
                        d_json, product, cat_id,
                        db_con, product_display, cursor, selection
                    )
                    b_product(cat_id, product, id_sub, db_con)
                    continuity(d_json, db_con)
